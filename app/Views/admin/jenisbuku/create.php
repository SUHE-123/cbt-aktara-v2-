...
<div class="d-flex form-wrapper">
  ...
  <div class="main-content flex-grow-1 d-flex flex-column">
    ...
    <div class="navbar-dashboard d-flex justify-content-between align-items-center px-4 py-3 bg-dark text-white shadow-sm z-3">
      ...
      <h4 class="mb-0"><strong>Tambah Jenis Buku</strong></h4>
      <div>
    ...
    <div class="container py-5 form-container mt-4 shadow">
        <form action="<?= base_url('/admin/jenisbuku/store') ?>" method="post">
        <?= csrf_field() ?>

        <div class="mb-3">
            <label for="nama_jenis_buku" class="form-label">Nama Jenis Buku</label>
            <input type="text" name="nama_jenis_buku" class="form-control" required>
        </div>

        

        <button type="submit" class="btn btn-success"><i class="bi bi-save"></i> Simpan</button>
        <a href="<?= base_url('/admin/jenisbuku') ?>" class="btn btn-secondary ms-2">Kembali</a>
        </form>
    </div>
    </div>
